...
class TranslateSelectionTests(GuiSubtitleTestCase):
    ...
    def test_selected_scene_is_translated_in_full(self) -> None:
        # Scene 1 and one of its batches, plus a batch from scene 2
        selection = ProjectSelection()
        selection.scenes[1] = SelectionScene(1, selected=True)
        selection.batches[(1, 1)] = SelectionBatch((1, 1), selected=False)
        selection.batches[(1, 2)] = SelectionBatch((1, 2), selected=True)
        ...
        self.assertLoggedEqual("scene 1 has no batch restriction", {}, scenes.get(1))
