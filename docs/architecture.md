...
- `TranscriptionLines` – `TranscriptionLineBuilder` turns a transcribed chunk into timed subtitle lines. Provider sub-segments (parts) are the lines when a provider returns them; otherwise parts are derived from the chunk transcript. The transcript is always the text, and word timings only time and split the parts. With words but no transcript, words are grouped into utterances instead. Brief slivers and overlapping lines are then merged. Pure logic with no provider or audio dependencies. Its limits come from a `LineSettings` dataclass, and it delegates to three helpers:
  - `TranscriptCutter` – cuts a chunk transcript into parts and times them from their words.
  - `UtteranceSplitter` – divides words at hard boundaries (pauses, speaker changes, sentence ends) and splits over-long runs at their best pause.
  - `LineMerger` – merges brief and overlapping lines into their neighbours, as dialogue where the speaker changes.
- `WordAlignment` – matches word timings to the transcript character by character (`AlignWords`), ignoring punctuation and spacing. A provider's `word_coverage` (`WordCoverage`) says whether its words can miss whole stretches of the transcript.
