...
- `TranscriptionLines` – `TranscriptionLineBuilder` turns a transcribed chunk into timed subtitle lines. Provider sub-segments (parts) are the lines when a provider returns them; words are matched to parts by text and used only to split a part exceeding the `max_characters` / `max_line_duration` options, taking each piece's timing from its words. Without parts, words are cut into utterances at pauses, speaker changes and sentence punctuation, and over-long utterances are split at their best pause (pause length weighted by centrality, with clause-punctuation bonuses and `min_split_chars` guarding fragments) rather than stranding a short tail. Brief slivers and overlapping lines are then merged in time order, with overlapping lines always shown as separate dialogue turns; a line still under `min_line_duration` is extended into the pause after it where `min_gap` allows. Words are capped at `max_line_duration`, since engines occasionally stamp one word across most of a chunk. Pure logic with no provider or audio dependencies. Word order comes from the engine, never from the timings, which are a best-effort signal.
