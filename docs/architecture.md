...
- `TranscriptionLines` – `TranscriptionLineBuilder` turns a transcribed chunk into timed subtitle lines. Provider sub-segments (parts) are the lines when a provider returns them. Providers that return only a transcript and words (Gemini, Qwen Local, OpenAI word timestamps) get parts derived from the transcript: it is cut at sentence punctuation (and speaker changes), or at pauses and speaker changes when it has no punctuation, and each part is timed from its words. The transcript is always the text source, since words drop characters and punctuation. Words are matched to parts character by character (`AlignWords`, ignoring punctuation and spacing), each respelled with the slice of part text it covers, and used only to time parts and to split a part exceeding the `max_characters` / `max_line_duration` options. Over-long parts are split at their best pause (pause length weighted by centrality, with clause-punctuation bonuses and `min_split_chars` guarding fragments) rather than stranding a short tail. Parts no word matches are placed between their neighbours by their share of characters, lasting about as long as their text takes to say (`EstimateSpeechSeconds`); a chunk with no usable words shares its span out the same way. Words are capped at a multiple of their estimated speaking time, and words far longer or far shorter than that are aligner failures and not used for timing. A provider's `word_coverage` (`WordCoverage`) says whether its words can miss whole stretches of the transcript (Qwen Local's forced aligner, `PARTIAL`); if so, a part its words spell sparsely is given room for the missed text, at the pace of its matched words and never overlapping its neighbours. Other providers' words (`COMPLETE`) time parts by themselves. With words but no transcript, words are grouped into utterances at pauses, speaker changes and sentence punctuation instead. Brief slivers and overlapping lines are then merged in time order, with overlapping lines always shown as separate dialogue turns; a line still under `min_line_duration` is extended into the pause after it where `min_gap` allows. Pure logic with no provider or audio dependencies. Word order comes from the engine, never from the timings, which are a best-effort signal. Muse returns turns without words, and its reported turn ends often run on over silence; its client splits each turn at its sentences, full stops included (`SentenceRanges` with `SentenceEnds.ALL`; transcripts with words leave full stops to them), anchoring the first to the turn start and the last to the turn end, and caps each at twice its `EstimateSpeechSeconds`.
